...
class Solution {
    public int minDominoRotations(int[] tops, int[] bottoms) {
        int candidate1 = validate(tops[0],tops,bottoms);
        if(candidate1!=-1) return candidate1;
        int candidate2 = validate(bottoms[0],tops,bottoms);
        return candidate2;
    }

    public int validate(int element,int [] tops,int []bottoms){
        int tops_swap = 0;
        int bottoms_swap = 0;

        for(int i=0;i<tops.length;i++){
            if (tops[i]!=element && bottoms[i]!=element) return -1;
            if(element!=tops[i]) tops_swap++;
            if(element!=bottoms[i]) bottoms_swap++;
        }
        return Math.min(tops_swap,bottoms_swap);
    }
}
